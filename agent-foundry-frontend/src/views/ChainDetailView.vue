<template>
  <div class="chain-detail mx-auto" style="max-width: 780px;">
    ...
    <template v-else-if="chain">
      <router-link to="/marketplace/chains" class="text-medium-emphasis text-decoration-none d-inline-block mb-4">← Back to AI Team Marketplace</router-link>
      ...
      <v-card variant="tonal" class="pa-4 mb-4">
        <h3 class="text-subtitle-1 text-medium-emphasis mb-2">Included AI Roles ({{ chain.agents?.length || 0 }})</h3>
      ...
      <div class="d-flex flex-wrap gap-3 mt-8">
        <template v-if="authStore.isAuthenticated">
          ...
          <template v-else-if="purchased">
            ...
            <v-btn color="primary" :to="`/chains/${chain.id}/edit`">Open AI Team</v-btn>
          </template>
        </template>
